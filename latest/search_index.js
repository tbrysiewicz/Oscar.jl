...

{
    ...
    "category": "section",
    "text": "The ring of integers is a Euclidean domain and Oscar provides Euclidean division through the functions divrem, div and rem.divrem(a::Oscar.Integer, b::Oscar.Integer) -> Oscar.Integer : division with remainder\ndiv(a::Oscar.Integer, b::Oscar.Integer) -> Oscar.Integer : quotient only\nrem(a::Oscar.Integer, b::Oscar.Integer) -> Oscar.Integer : remainder onlyOne or both arguments may be Julia integers, however if they both are, the result will be a Julia integer not an Oscar integer.Integer Euclidean division of a by b computes a quotient and remainder such thata = qb + rwhere r  b. When r neq 0 the sign of r will be the same as the sign of a.mod(a::Oscar.Integer, b::Oscar.Integer) -> Oscar.Integer : remainder onlyOne or both arguments may be Julia integers, however if they both are, the result will be a Julia integer not an Oscar integer.Returns a remainder which is either 0 or the same sign as b. Thus, if b  0 then mod(a, b) will be in the range 0 b). There is no function implemented to compute the corresponding quotient.remainder division sign rounding\nrem div/divrem same as dividend towards zero\nmod  same as divisor towards -inftyq, r = divrem(ZZ(5), ZZ(3))\nq = div(ZZ(7), ZZ(2))\nr = mod(ZZ(4), ZZ(3))All three functions raise an exception if the modulus b is zero.note: Note\nThe rem function does not provide a minimal set of representatives, e.g. rem(-2, 3) = -2 but rem(1, 3) = 1."
},
